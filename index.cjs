...
async function register(nets, computes, roles) {
...
async function acquireLock(resource, owner) {
  ...
  const lock = JSON.stringify({
    owner,
    acquireAt: new Date().toISOString(),
    expireAt: Date.now() + LOCK_TIMEOUT * 1000
  });
...
async function getResults() {
  ...
  return results.split('\n').filter(x => x).reverse().map(r => {
    try { var p = JSON.parse(r); return '📥 From ' + p.from + ': ' + (p.result || '(empty)'); } catch { return r; }
  }).join('\n');
}
...
async function getMyTasks() {
  const tasks = await redisCmd('LRANGE', 'tasks:' + AGENT_NAME, '0', '99');
  if (!tasks) return 'No tasks';
  return tasks.split('\n').filter(x => x).reverse().map(t => {
    try { var p = JSON.parse(t); return (p.status==='completed'?'✅':p.status==='failed'?'❌':'⏳') + ' [' + p.status + '] ' + p.from + ': ' + p.task + '\n   ID: ' + p.id + ' | Attempts: ' + p.attempts; } catch { return t; }
  }).join('\n');
}
...
async function getHistory(limit) {
  ...
  return events.split('\n').filter(x => x).reverse().map(e => {
    try { var p = JSON.parse(e); return '[' + p.timestamp.slice(0,16) + '] ' + p.agent + ': ' + p.type; } catch { return e; }
  }).join('\n');
}
...
async function memories(limit) {
  ...
  return mems.split('\n').reverse().filter(x => x).map(m => {
    try { var p = JSON.parse(m); return '[' + p.timestamp.slice(0,16) + '] ' + p.agent + ': ' + p.content; } catch { return m; }
  }).join('\n');
}
...
const skills = {
  'redis-agents': {
    ...
    params: [{ name: 'detailed', type: 'boolean', required: false }],
    handler: async function(args) { return listAgents(args.detailed); }
  },
  'redis-find': {
    ...
    handler: async function(args) { return find(args.capability, args.type||'all'); }
  },
  'redis-register': {
    description: 'Register this agent',
    params: [
      { name: 'network', type: 'string', required: false },
      { name: 'compute', type: 'string', required: false },
      { name: 'role', type: 'string', required: false }
    ],
    handler: async function(args) { return register(args.network, args.compute, args.role); }
  },
  'redis-tasks': {
    description: 'Get my tasks',
    handler: async function() { return getMyTasks(); }
  },
  'redis-results': {
    description: 'Get results sent to me',
    handler: async function() { return getResults(); }
  },
  'redis-send': {
    ...
    handler: async function(args) { return sendTask(args.to, args.task, args.priority||'normal'); }
  },
  'redis-complete': {
    ...
    handler: async function(args) { return completeTask(args.taskId, args.result); }
  },
  'redis-retry': {
    description: 'Retry failed task',
    params: [{ name: 'taskId', type: 'string', required: true }],
    handler: async function(args) { return retryTask(args.taskId); }
  },
  'redis-memory': {
    description: 'Share memory',
    params: [{ name: 'content', type: 'string', required: true }],
    handler: async function(args) { return shareMemory(args.content); }
  },
  'redis-memories': {
    description: 'View memories',
    params: [{ name: 'limit', type: 'number', required: false }],
    handler: async function(args) { return memories(args.limit); }
  },
  'redis-history': {
    description: 'View global history',
    params: [{ name: 'limit', type: 'number', required: false }],
    handler: async function(args) { return getHistory(args.limit); }
  },
  'redis-status': {
    description: 'Get my status',
    handler: async function() {
      const d = await redisCmd('GET', 'agent:'+AGENT_NAME);
      return d ? JSON.stringify(JSON.parse(d), null, 2) : 'Not registered';
    }
  }
};
...
if (require.main === module) {
  ...
  if (cmd === 'agents') listAgents(args.includes('--detailed')).then(console.log).catch(e=>console.error(e.message));
  else if (cmd === 'find') find(args[1], args[2]||'all').then(console.log).catch(e=>console.error(e.message));
  else if (cmd === 'register') register(args[1], args[2], args[3]).then(()=>console.log('✅ Registered')).catch(e=>console.error(e.message));
    ...
    register().then(() => console.log('✅ Registered'));
...
if (REDIS_HOST && REDIS_PASSWORD && require.main === module) {
  register().then(() => console.log('✅ ' + AGENT_NAME + ' registered'));
  setInterval(() => heartbeat().catch(() => {}), HEARTBEAT_INTERVAL * 1000);
}
